  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <Input
        ...
        hint="Employees use numeric PIN (default sample: 1234). Bootstrap admin uses ADMIN / shaswat."
        required
      />
      {error ? <p className="text-sm text-danger">{error}</p> : null}
      <Button type="submit" disabled={submitting} className="w-full rounded-xl py-2.5 text-sm font-semibold">
