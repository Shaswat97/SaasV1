...
const SEEDED_ADMIN_CODE = "ADMIN";
const SEEDED_ADMIN_NAME = "Admin";
const SEEDED_ADMIN_PASSWORD = "shaswat";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      // Ensure baseline warehouse + operational zones so the app is usable immediately after reset.
      ...
      const defaultZones = [
        { code: "ZN-RAW", name: "Raw Material", type: "RAW_MATERIAL" },
        { code: "ZN-WIP", name: "Processing / WIP", type: "PROCESSING_WIP" },
        { code: "ZN-FG", name: "Finished Goods", type: "FINISHED" },
        { code: "ZN-SCRAP", name: "Scrap", type: "SCRAP" },
        { code: "ZN-TRANSIT", name: "In Transit", type: "IN_TRANSIT" }
      ] as const;

      const zones = await Promise.all(
        defaultZones.map((zone) =>
          tx.zone.upsert({
            where: { companyId_code: { companyId, code: zone.code } },
            update: {
      ...
      // Always keep a predictable admin login after sample-data load.
      const seededAdmin = await tx.employee.upsert({
        where: { companyId_code: { companyId, code: SEEDED_ADMIN_CODE } },
        update: {
          name: SEEDED_ADMIN_NAME,
          pinHash: hashPin(SEEDED_ADMIN_PASSWORD),
          pinUpdatedAt: new Date(),
          active: true,
          deletedAt: null
        },
      ...
      await tx.employeeRole.upsert({
        where: { employeeId_roleId: { employeeId: seededAdmin.id, roleId: adminRole.id } },
        update: {},
        create: { employeeId: seededAdmin.id, roleId: adminRole.id }
      });
      ...
      const employeeCount = new Set([...employees.map((employee) => employee.id), seededAdmin.id]).size;
