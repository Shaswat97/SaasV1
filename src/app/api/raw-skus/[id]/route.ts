...
export async function GET(_: Request, { params }: { params: { id: string } }) {
  ...
  if (!sku) return jsonError("Raw SKU not found", 404);
...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  ...
  if (!existing) return jsonError("Raw SKU not found", 404);
  ...
  try {
    ...
    await recordActivity({
      ...
      entityType: "Raw SKU",
      entityId: sku.id,
      summary: `Updated raw SKU ${sku.code} · ${sku.name}.`
    });
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  ...
  await recordActivity({
    ...
    entityType: "Raw SKU",
    entityId: sku.id,
    summary: `Deleted raw SKU ${sku.code} · ${sku.name}.`
  });
