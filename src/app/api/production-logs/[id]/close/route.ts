...
const closeSchema = z.object({
  ...
  machineOverheadCost: z.number().min(0).optional(),
...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  const laborCostInput = data.laborCost ?? 0;
  const machineOverheadCostInput = data.machineOverheadCost ?? 0;
  ...
  const goodTargetRemaining = Math.max(log.plannedQty - (log.goodQty ?? 0), 0);
  if (data.goodQty > goodTargetRemaining) {
    return jsonError(
      `Good quantity (${data.goodQty}) exceeds open target quantity (${goodTargetRemaining}) for this log`,
      400
    );
  }
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      const closeMoment = new Date(data.closeAt);
      ...
      const overrunQty = Math.max(totalQty - wipQtyOnHand, 0);
      if (overrunQty > 0) {
        await recordStockMovement(
          {
            ...
            referenceId: log.id,
            notes: "Production overrun top-up (actual reject/scrap exceeded planned WIP)"
          },
          tx
        );
        await tx.productionLogAudit.create({
          data: {
            companyId,
            logId: log.id,
            action: "OVERRUN_CLOSE",
            reason: `Closed with total output ${totalQty} against WIP ${wipQtyOnHand}; overrun top-up ${overrunQty}. Good ${data.goodQty}, reject ${data.rejectQty}, scrap ${data.scrapQty}.`
          }
        });
      }
      ...
      const bomForClose = await tx.bom.findFirst({
        where: { companyId, finishedSkuId: log.finishedSkuId, deletedAt: null },
        include: { lines: true },
        orderBy: { version: "desc" }
      });
      if (bomForClose?.lines?.length && (!data.rawConsumptions || data.rawConsumptions.length === 0)) {
        throw new Error("Raw consumption is required to close this log because a BOM is mapped for the finished SKU");
      }
      ...
      if (data.rawConsumptions?.length) {
        ...
        const rawSkuIds = Array.from(new Set(data.rawConsumptions.map((entry) => entry.rawSkuId)));
        ...
        if (bomForClose?.lines?.length) {
          const expectedByRawSku = new Map<string, number>();
          for (const line of bomForClose.lines) {
            const expectedQty = totalQty * line.quantity;
            expectedByRawSku.set(line.rawSkuId, (expectedByRawSku.get(line.rawSkuId) ?? 0) + expectedQty);
          }
          const actualByRawSku = new Map<string, number>();
          for (const entry of data.rawConsumptions) {
            actualByRawSku.set(entry.rawSkuId, (actualByRawSku.get(entry.rawSkuId) ?? 0) + entry.quantity);
          }
      ...
      let actualTotalCost: number | null = null;
      const conversionCost = laborCostInput + machineOverheadCostInput;
      ...
      if (remainingAfter === 0) {
              ...
              const movement = await recordStockMovement(
                {
                  ...
                  referenceType: "PROD_LOG",
                  referenceId: log.id,
                  notes: "Production actual raw reconciliation (extra issue at close)"
                },
                tx
              );
              ...
              const movement = await recordStockMovement(
                {
                  ...
                  referenceType: "PROD_LOG",
                  referenceId: log.id,
                  notes: "Production actual raw reconciliation (unused raw returned at close)"
                },
                tx
              );
              adjustmentCostDelta -= movement.totalCost;
        ...
        actualTotalCost = (actualRawCost ?? 0) + conversionCost;
      ...
      const updated = await tx.productionLog.update({
        ...
        data: {
          ...
          machineOverheadCost: machineOverheadCostInput,
          conversionCost,
          actualTotalCost,
