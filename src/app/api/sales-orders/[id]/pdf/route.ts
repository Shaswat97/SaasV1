...
export async function GET(_: Request, { params }: { params: { id: string } }) {
  ...
  if (!order) return new Response("Sales order not found", { status: 404 });
  ...
  const totalsHtml = `
    <table class="totals">
      <tbody>
        ...
        <tr><td><strong>Order Total</strong></td><td class="num"><strong>${esc(formatMoney(total))}</strong></td></tr>
      </tbody>
    </table>
  `;
  ...
  const partyBlock = `
    ...
    <div class="meta">Currency: ${esc(order.currency)}</div>
  `;

  const html = renderDocumentShell({
    title: "Sales Order",
    docNumber: order.soNumber ?? order.id,
