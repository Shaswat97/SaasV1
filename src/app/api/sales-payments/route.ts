...
export async function POST(request: Request) {
  ...
  if (invoice.deliveryId && invoice.lines.length > 0) {
    ...
    if (invoiceQtyBySoLine.size > 0) {
      const soLineIds = Array.from(invoiceQtyBySoLine.keys());
      const [orderLines, relatedInvoiceLines] = await Promise.all([
        prisma.salesOrderLine.findMany({
          where: { id: { in: soLineIds }, salesOrder: { companyId } },
          select: { id: true, deliveredQty: true }
        }),
        prisma.salesInvoiceLine.findMany({
          where: {
            soLineId: { in: soLineIds },
            invoice: { companyId, salesOrderId: invoice.salesOrderId }
          },
          include: {
      ...
      const deliveredBySoLine = new Map(orderLines.map((line) => [line.id, line.deliveredQty ?? 0]));
      const otherInvoicedBySoLine = new Map<string, number>();
      relatedInvoiceLines.forEach((line) => {
        if (line.invoice.id === invoice.id) return;
        otherInvoicedBySoLine.set(line.soLineId, (otherInvoicedBySoLine.get(line.soLineId) ?? 0) + line.quantity);
      });

      const fullyCoveredByOtherInvoices = soLineIds.every((soLineId) => {
        const delivered = deliveredBySoLine.get(soLineId) ?? 0;
        const otherInvoiced = otherInvoicedBySoLine.get(soLineId) ?? 0;
        return otherInvoiced >= delivered;
      });

      if (fullyCoveredByOtherInvoices) {
        return jsonError(
          "This delivery invoice is already covered by another invoice for the same delivered quantity. Void the duplicate invoice instead of collecting payment.",
          400
        );
      }
    }
  ...
  const currentBalance = Math.abs(invoice.balanceAmount) < CURRENCY_EPSILON ? 0 : invoice.balanceAmount;
  if (currentBalance <= 0) return jsonError("Invoice already paid", 400);
  if (parsed.data.amount - currentBalance > CURRENCY_EPSILON) return jsonError("Payment exceeds outstanding balance", 400);
  ...
  const updated = await prisma.$transaction(async (tx) => {
    ...
    const rawNewBalance = currentBalance - parsed.data.amount;
    const newBalance = rawNewBalance <= CURRENCY_EPSILON ? 0 : rawNewBalance;
