...
function formatMovementSource(referenceType?: string | null, referenceId?: string | null) {
  if (!referenceType && !referenceId) return "Manual / Unlinked";
  const typeLabel = referenceType ? (referenceTypeLabels[referenceType] ?? referenceType) : "Unlinked";
  return referenceId ? `${typeLabel} · ${referenceId}` : typeLabel;
}
...
export default function InventoryPage() {
  ...
  const summaryRawBatchRows = useMemo(() => {
    if (!selectedSummary) return [];
    if (!selectedSummary.zoneTypes.includes("RAW_MATERIAL")) return [];
    return rawBatches.slice().sort((a, b) => new Date(a.receivedAt).getTime() - new Date(b.receivedAt).getTime());
  }, [selectedSummary, rawBatches]);
  ...
  const movementRows = useMemo(
    () =>
      filteredLedger.map((entry) => ({
        ...
        flow: (
          <div className="flex min-w-[220px] items-center gap-2 whitespace-normal">
            <Badge label={entry.direction === "IN" ? "IN" : "OUT"} variant={entry.direction === "IN" ? "success" : "warning"} />
            <span className="text-sm">{movementTypeLabels[entry.movementType] ?? entry.movementType}</span>
          </div>
        ),
        source: (
          <div className="min-w-[220px] whitespace-normal text-xs text-text-muted">
            {formatMovementSource(entry.referenceType, entry.referenceId)}
          </div>
        ),
        qty: `${entry.direction === "IN" ? "+" : "-"}${formatQty(entry.quantity, entry.sku.unit)}`,
    ...
    <div className="flex flex-col gap-8">
      ...
      >
        <div className="space-y-6">
          ...
          {summaryRawBatchRows.length > 0 ? (
            <div className="space-y-3">
              <div>
                <p className="text-sm font-medium text-text">Raw Material Batch Inventory</p>
                <p className="text-xs text-text-muted">
                  Batch-wise inwarded stock currently available in raw material zone (FIFO visibility).
                </p>
              </div>
              <DataTable
                columns={[
                  ...
                  { key: "remainingQty", label: "Remaining Qty", align: "right" },
                  { key: "cost", label: "Cost / Unit", align: "right" },
                  { key: "value", label: "Remaining Value", align: "right" }
                ]}
                rows={summaryRawBatchRows.map((batch) => ({
                  sku: `${batch.sku.code} · ${batch.sku.name}`,
                  batch: batch.batchNumber,
                  receivedAt: new Date(batch.receivedAt).toLocaleDateString("en-IN"),
                  receivedQty: formatQty(batch.quantityReceived, batch.sku.unit),
                  remainingQty: formatQty(batch.quantityRemaining, batch.sku.unit),
                  cost: `${currency.format(batch.costPerUnit)} / ${batch.sku.unit}`,
                  value: currency.format(batch.quantityRemaining * batch.costPerUnit)
                }))}
                emptyLabel="No active raw material batches found."
              />
            </div>
          ) : null}
