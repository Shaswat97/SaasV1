...
type ProductionLog = {
  ...
  machineOverheadCost?: number | null;
...
function buildRawConsumptionRows(
  ...
  const batchRemainingById = new Map(rawBatches.map((batch) => [batch.id, batch.quantityRemaining]));
  const plannedByRawSku = new Map<string, number>();

  // Aggregate duplicate BOM lines of the same raw SKU before allocating batches.
  // This prevents unnecessary split rows when only one batch exists for that raw SKU.
  bom.lines.forEach((line) => {
    const plannedRawQty = baseQty ? baseQty * line.quantity : 0;
    plannedByRawSku.set(line.rawSkuId, (plannedByRawSku.get(line.rawSkuId) ?? 0) + plannedRawQty);
  });
...
export default function ProductionPage() {
  ...
  const [closeMachineOverheadCost, setCloseMachineOverheadCost] = useState("");
  ...
  const selectedLineOpenQty = selectedLine?.openQty ?? 0;
  const selectedLineKey = selectedLine?.id ?? "";

  useEffect(() => {
    if (purpose !== "ORDER") return;
    if (!selectedLineKey) return;
    const remaining = Math.max(Math.floor(selectedLineOpenQty), 0);
    setPlannedQty(remaining > 0 ? String(remaining) : "");
  }, [purpose, selectedLineKey, selectedLineOpenQty]);
  ...
  const closeProcessRemainingQty = useMemo(() => {
    if (!selectedCloseLog) return 0;
    const processed =
      Number(selectedCloseLog.goodQty ?? 0) +
      Number(selectedCloseLog.rejectQty ?? 0) +
      Number(selectedCloseLog.scrapQty ?? 0);
    return Math.max(selectedCloseLog.plannedQty - processed, 0);
  }, [selectedCloseLog]);

  const rawConsumptionBasisQty = useMemo(
    () => (closeTotalQty > 0 ? closeTotalQty : closeRemainingQty),
    [closeTotalQty, closeRemainingQty]
  );

  const rawConsumptionCostPreview = useMemo(
    () =>
      rawConsumptions.reduce((sum, row) => {
        const qty = Number(row.quantity || 0);
        if (!Number.isFinite(qty) || qty <= 0) return sum;
        const batchCost = row.batchId ? rawBatchMap.get(row.batchId)?.costPerUnit ?? 0 : 0;
        return sum + qty * batchCost;
      }, 0),
    [rawConsumptions, rawBatchMap]
  );
  const closeOverrunQty = useMemo(() => Math.max(closeTotalQty - closeProcessRemainingQty, 0), [closeTotalQty, closeProcessRemainingQty]);
  const rawConsumptionMismatchPreview = useMemo(() => {
  ...
  const laborCostPreview = useMemo(() => {
    const value = Number(closeLaborCost || 0);
    return Number.isFinite(value) && value >= 0 ? value : 0;
  }, [closeLaborCost]);

  const machineOverheadCostPreview = useMemo(() => {
    const value = Number(closeMachineOverheadCost || 0);
    return Number.isFinite(value) && value >= 0 ? value : 0;
  }, [closeMachineOverheadCost]);

  const conversionCostPreview = laborCostPreview + machineOverheadCostPreview;
  const totalActualCostPreview = rawConsumptionCostPreview + conversionCostPreview;
  const totalOutputQtyPreview = Number(goodQty || 0) + Number(rejectQty || 0) + Number(scrapQty || 0);
  const goodUnitCostPreview = Number(goodQty || 0) > 0 ? totalActualCostPreview / Number(goodQty) : null;
  ...
  useEffect(() => {
    ...
    setRawConsumptions(buildRawConsumptionRows(selectedCloseLog, bomMap, rawBatches, rawConsumptionBasisQty));
  }, [selectedCloseLog, rawRowsAuto, rawConsumptionBasisQty, bomMap, rawBatches]);

  const resetRawConsumptionsFromBom = () => {
    if (!selectedCloseLog) return;
    setRawConsumptions(buildRawConsumptionRows(selectedCloseLog, bomMap, rawBatches, rawConsumptionBasisQty));
    setRawRowsAuto(true);
  };
  ...
  async function startLog(event: FormEvent) {
    ...
    if (purpose === "ORDER" && selectedLine && qty > selectedLine.openQty) {
      push(
        "error",
        `Planned quantity (${qty}) cannot exceed remaining open quantity (${selectedLine.openQty}) for this order line`
      );
      return;
    }
  ...
  async function closeLog(event: FormEvent) {
    ...
    if (!closeAt.trim()) {
      push("error", "Close Time is required to close the log");
      return;
    }
    const good = Number(goodQty);
    const reject = Number(rejectQty || 0);
    const scrap = Number(scrapQty || 0);
    const laborCost = Number(closeLaborCost || 0);
    const machineOverheadCost = Number(closeMachineOverheadCost || 0);
    ...
    if (!Number.isFinite(laborCost) || laborCost < 0 || !Number.isFinite(machineOverheadCost) || machineOverheadCost < 0) {
      push("error", "Labor and machine overhead costs must be 0 or more");
      return;
    }
    const totalOutput = good + reject + scrap;
    if (good > closeRemainingQty) {
      push(
        "error",
        `Good Qty (${good}) cannot exceed open target quantity (${closeRemainingQty}). Reject/Scrap can be extra if raw consumption is available.`
      );
      return;
    }
    const closeAtIso = new Date(closeAt);
    if (Number.isNaN(closeAtIso.getTime())) {
      push("error", "Close Time must be a valid date and time");
      return;
    }
    ...
    if (consumptionPayload.some((row) => !rawSkuMap.has(row.rawSkuId))) {
      push("error", "Raw consumption rows are stale or invalid. Click 'Reset from BOM' and try again.");
      return;
    }
    if (consumptionPayload.some((row) => row.batchId && !rawBatchMap.has(row.batchId))) {
      push("error", "One or more selected raw batches are no longer available. Reset raw rows and reselect.");
      return;
    }
    ...
    if (consumptionPayload.some((row) => !Number.isFinite(row.quantity) || row.quantity < 0)) {
      push("error", "Raw consumption quantities cannot be negative");
      return;
    }
    const bom = selectedCloseLog ? bomMap.get(selectedCloseLog.finishedSku.id) : undefined;
    if (bom?.lines?.length) {
      if (!consumptionPayload.length) {
    ...
    try {
      await apiSend(`/api/production-logs/${closeLogId}/close`, "POST", {
        ...
        laborCost,
        machineOverheadCost,
        closeAt: closeAtIso.toISOString(),
  ...
  function preventAccidentalCloseSubmit(event: ReactKeyboardEvent<HTMLFormElement>) {
    if (event.key !== "Enter") return;
    const target = event.target as HTMLElement | null;
    if (!target) return;
    const tag = target.tagName;
    // Prevent Enter on inputs/selects from submitting the close form accidentally.
      ...
      <div className="grid gap-6 lg:grid-cols-[1.2fr_1.8fr]">
            ...
            <form className="space-y-4" onSubmit={startLog}>
              ...
              {purpose === "ORDER" && selectedLine ? (
                <div className="rounded-2xl border border-border/70 bg-bg-subtle/80 p-3 text-xs text-text-muted">
                  <div>
                    Remaining qty: <span className="font-medium text-text">{selectedLine.openQty}</span> {selectedLine.unit} · Order{" "}
                    {formatOrderNumber(selectedLine.soNumber, selectedLine.id)}
                  </div>
                  <div className="mt-1">
                    Ordered: {selectedLine.orderedQty} {selectedLine.unit} · Produced (linked logs): {selectedLine.producedQty} {selectedLine.unit}
                  </div>
                </div>
              ) : null}
              <Select
              ...
              <Input
                ...
                type="number"
                min={1}
                max={purpose === "ORDER" && selectedLine ? selectedLine.openQty : undefined}
                value={plannedQty}
              ...
              {purpose === "ORDER" && selectedLine ? (
                <div className="text-xs text-text-muted">
                  Enter shift target up to remaining order qty ({selectedLine.openQty} {selectedLine.unit}).
                </div>
              ) : null}
        ...
        <div className="flex flex-col gap-6">
          <Card>
            ...
            <CardBody>
              <form className="space-y-4" onSubmit={closeLog} onKeyDown={preventAccidentalCloseSubmit}>
                <Select
                  label="Open Log"
                  value={closeLogId}
                ...
                />
                {selectedCloseLog ? (
                  <div className="rounded-xl border border-border/60 bg-bg-subtle/60 px-3 py-2 text-xs text-text-muted">
                    Close validation: `Good Qty` must stay within the open target for this log. Reject/Scrap may exceed the target if raw material is available and mapped BOM quantities are entered.
                    {" "}Open good target:{" "}
                    <span className="font-medium text-text">
                      {closeRemainingQty} {selectedCloseLog.finishedSku.unit}
                    </span>
                    {" "}({selectedCloseLog.plannedQty} planned)
                  </div>
                ) : null}
                {closeOverrunQty > 0 ? (
                  <div className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
                    Total output is {closeTotalQty} ({closeOverrunQty} above the open target-processing qty). This will be recorded as a production overrun (audited) if raw consumption matches the BOM.
                  </div>
                ) : null}
                <div className="space-y-3 rounded-2xl border border-border/60 bg-bg-subtle/70 p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-text-muted">
                        Basis = Good + Reject + Scrap ({Number.isFinite(rawConsumptionBasisQty) ? rawConsumptionBasisQty : 0}).{" "}
                        {rawRowsAuto ? "Auto-updates while you change output quantities." : "Manual mode: click Reset from BOM after changing output quantities."}
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      <span
                        className={`rounded-full border px-2 py-1 text-[11px] font-medium ${rawRowsAuto
                          ? "border-emerald-200 bg-emerald-50 text-emerald-700"
                          : "border-amber-200 bg-amber-50 text-amber-700"
                          }`}
                      >
                        {rawRowsAuto ? "Auto" : "Manual"}
                      </span>
                      <Button type="button" variant="ghost" onClick={resetRawConsumptionsFromBom} disabled={!selectedCloseLog}>
                        Reset from BOM
                      </Button>
                  ...
                  {(() => {
                    const splitSummary = new Map<string, { rows: number; totalQty: number }>();
                    rawConsumptions.forEach((row) => {
                      if (!row.rawSkuId) return;
                      const qty = Number(row.quantity || 0);
                      const current = splitSummary.get(row.rawSkuId) ?? { rows: 0, totalQty: 0 };
                      splitSummary.set(row.rawSkuId, {
                        rows: current.rows + 1,
                        totalQty: current.totalQty + (Number.isFinite(qty) ? qty : 0)
                      });
                    });
                    const splitRows = Array.from(splitSummary.entries()).filter(([, entry]) => entry.rows > 1);
                    if (!splitRows.length) return null;
                    return (
                      <div className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
                        Some raw items are split across multiple batches based on available lot quantities (FIFO). This is normal when one batch is not enough.
                      </div>
                    );
                  })()}
                  ...
                  {rawConsumptions.length === 0 ? (
                    ...
                    <div className="space-y-3">
                      {rawConsumptions.map((row, index) => {
                        ...
                        const sameSkuRows = rawConsumptions.filter((item) => item.rawSkuId === row.rawSkuId);
                        const isSplitSku = row.rawSkuId && sameSkuRows.length > 1;
                        const isFirstSplitRow =
                          isSplitSku && rawConsumptions.findIndex((item) => item.rawSkuId === row.rawSkuId) === index;
                        return (
                          <div
                            key={`${row.rawSkuId}-${index}`}
                            className="space-y-2 rounded-xl border border-border/60 bg-surface/70 p-3"
                          >
                                  ...
                                  <>
                                    {isFirstSplitRow ? (
                                      <span className="inline-flex items-center rounded-full border border-amber-200 bg-amber-50 px-2 py-0.5 text-amber-700">
                                        Split across {sameSkuRows.length} batches
                                      </span>
                                    ) : (
                                      <span className="inline-flex items-center rounded-full border border-border/60 bg-bg-subtle px-2 py-0.5">
                                        Same raw SKU (batch split)
                                      </span>
                                    )}
                                  </>
                                ) : (
                            ...
                            <div className="grid gap-3 lg:grid-cols-[1.9fr_2.2fr_1fr_1fr]">
                              <Select
                                label="Raw SKU"
                                value={row.rawSkuId}
                                onChange={(event) => {
                ...
                <div className="grid gap-4 lg:grid-cols-2">
                  <Input
                    label="Labor Cost (INR)"
                    type="number"
                    min="0"
                    step="0.01"
                    value={closeLaborCost}
                    onChange={(event) => setCloseLaborCost(event.target.value)}
                    hint="Operator / helper labor cost for this production close."
                  />
                  <Input
                    label="Machine Overhead Cost (INR)"
                    type="number"
                    min="0"
                    step="0.01"
                    value={closeMachineOverheadCost}
                    onChange={(event) => setCloseMachineOverheadCost(event.target.value)}
                    hint="Electricity, machine running overhead, consumables, etc."
                  />
                </div>
                <div className="rounded-2xl border border-border/60 bg-bg-subtle/70 p-4">
                  <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
                    <div>
                      <div className="text-[11px] uppercase tracking-[0.12em] text-text-muted">Raw Cost (Actual)</div>
                      <div className="mt-1 text-sm font-semibold text-text">{inrCurrency.format(rawConsumptionCostPreview)}</div>
                    </div>
                    <div>
                      <div className="text-[11px] uppercase tracking-[0.12em] text-text-muted">Labor Cost</div>
                      <div className="mt-1 text-sm font-semibold text-text">{inrCurrency.format(laborCostPreview)}</div>
                    </div>
                    <div>
                      <div className="text-[11px] uppercase tracking-[0.12em] text-text-muted">Machine Overhead</div>
                      <div className="mt-1 text-sm font-semibold text-text">{inrCurrency.format(machineOverheadCostPreview)}</div>
                    </div>
                    <div>
                      <div className="text-[11px] uppercase tracking-[0.12em] text-text-muted">Conversion Cost</div>
                      <div className="mt-1 text-sm font-semibold text-text">{inrCurrency.format(conversionCostPreview)}</div>
                    </div>
                    <div>
                      <div className="text-[11px] uppercase tracking-[0.12em] text-text-muted">Total Actual Cost</div>
                      <div className="mt-1 text-sm font-semibold text-text">{inrCurrency.format(totalActualCostPreview)}</div>
                      <div className="mt-1 text-xs text-text-muted">
                        {goodUnitCostPreview != null
                          ? `≈ ${inrCurrency.format(goodUnitCostPreview)} / good unit`
                          : totalOutputQtyPreview > 0
                            ? "Enter Good Qty to view cost per good unit"
                            : "Set output quantities to preview unit cost"}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <Button
                  ...
                  disabled={
                    ...
                    Number(goodQty) <= 0 ||
                    !closeAt.trim() ||
                    Number(goodQty || 0) > closeRemainingQty ||
                    Boolean(rawConsumptionMismatchPreview)
                  }
                >
                  Close Log
                </Button>
            ...
            <DataTable
              columns={[
                ...
                { key: "good", label: "Good", align: "right" },
                {
                  ...
                  label: "Scrap",
                  align: "right",
                  tooltip: "Total lost output in this log (Reject Qty + Scrap Qty)."
                },
                {
                  key: "variance",
                  label: "Var",
                  align: "right",
                  tooltip:
                    "Time variance in minutes: actual runtime minus standard runtime (based on planned qty and machine capacity). Positive means delay."
                },
                {
                  key: "materialVar",
                  label: "Material Var%",
                  align: "right",
                  tooltip:
                    "Material variance percent vs BOM/expected material usage or cost. Positive means higher-than-expected material consumption/cost."
                },
                {
                  key: "rate",
                  label: "Units/hr",
                  align: "right",
                  tooltip: "Good units produced per hour for this log (Good Qty divided by runtime from Start to Close)."
                },
                {
                  key: "oee",
                  label: "OEE",
                  align: "right",
                  tooltip:
                    "Current production efficiency score for this log. In this screen it is calculated from good output against planned quantity."
                },
